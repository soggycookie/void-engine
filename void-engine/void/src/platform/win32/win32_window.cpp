...
created on the same thread. Therefore, avoid lengthy processing inside your
window procedure. For example, suppose your program opens a TCP connection and
waits indefinitely for the server to respond. If you do that inside the window
procedure, your UI will not respond until the request completes. During that
time, the window cannot process mouse or keyboard input, repaint itself, or even
close.
https://learn.microsoft.com/en-us/windows/win32/learnwin32/writing-the-window-procedure
*/

static LRESULT CALLBACK WindowProcedure(HWND hwnd, UINT uMsg, WPARAM wParam,
                                        LPARAM lParam)
{
    LRESULT result = 0;
    Window *window =
    ...
    case WM_XBUTTONDOWN:
    {
        ...
        InputEvent e(InputEventType::MOUSE_PRESSED, voidBtn, 0, 0);
        window->DispatchInputEvent(e);
...
static void RegisterWindowClass(HINSTANCE hInst, const char *className)
{
    ...
    if (atom == 0)
    {
        ...
        MessageBoxA(NULL, buf, "Window Class Registration Failed",
                    MB_OK | MB_ICONERROR);
    }
}
...
void Win32Window::Update()
{
    ...
    {
        m_deltaTime = static_cast<double>(m_currCount - m_prevCount) /
...
void Win32Window::EndTimeElapse(double &outPassedTime)
{
    ...
    outPassedTime = static_cast<double>(deltaCount) /
                    static_cast<double>(m_countsPerSec.QuadPart);
}
...
//     {
//         MessageBoxA(m_windowHandle, "D3D11 failed to initialize", "Error",
//         MB_OK); PostMessage(m_windowHandle, WM_CLOSE, 0, 0); return false;
//     }
...
//    {
//        MessageBoxA(m_windowHandle, "CPU does not support DirectXMath",
//        "Error", MB_OK); PostMessage(m_windowHandle, WM_CLOSE, 0, 0);
