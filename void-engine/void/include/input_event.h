...
public:
    InputEvent()
        : m_category(InputEventCategory::NONE), m_type(InputEventType::NONE),
          m_keyBtn(KeyCode::NONE), m_mousePos(), m_isHandled(false)
    {
    }

    InputEvent(InputEventCategory category, InputEventType type, int32_t x,
               int32_t y)
        : m_category(category), m_type(type), m_keyBtn(KeyCode::NONE),
          m_mousePos(x, y), m_isHandled(false)
    {
    }

    // keyboard
    InputEvent(InputEventType type, KeyCode btn)
    ...
    // mouse
    InputEvent(InputEventType type, KeyCode btn, int32_t x, int32_t y)
    ...
    KeyCode KeyBtn() const { return m_keyBtn; }
