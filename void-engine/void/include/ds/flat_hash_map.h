...
public:
    ...
    public:
        bool IsValid() const { return m_ptr->header.occupied; }

        Value &GetValue() { return *(reinterpret_cast<Value *>(m_ptr->value)); }

        Key &GetKey() { return *(reinterpret_cast<Key *>(m_ptr->key)); }

        const Value &GetValue() const { return *(reinterpret_cast<Value *>(m_ptr->value)); }

        const Key &GetKey() const { return *(reinterpret_cast<Key *>(m_ptr->key)); }
        ...
        Iterator operator-(int64_t index) const { return Iterator(m_ptr - index); }
...
public:
    ...
    FlatHashMap(FreeListAllocator *allocator) : FlatHashMap(allocator, DEFAULT_BUCKET) {}
        ...
        : m_allocator(allocator), m_bucketCount(bucketCount), m_count(0)
    {
        static_assert((std::is_move_constructible_v<Key> || std::is_copy_constructible_v<Key>) &&
                          std::is_destructible_v<Key>,
                      "Key must be movable or copyable! [FlatHashMap]");
        static_assert(std::is_trivially_copyable_v<Value> && std::is_move_constructible_v<Value> &&
                          std::is_trivially_destructible_v<Value>,
                      "Value must be POD only! [FlatHashMap]");
        ...
        m_data = static_cast<uint8_t *>(
            allocator->Alloc(m_alignedBucketSize * m_bucketCount, bucketAlign));
    ...
    bool ContainsKey(const Key &key) const
    {
        ...
        while (true)
        {
            ...
            PSL++;
            index++;
            bucket = reinterpret_cast<Bucket *>(m_data + (index & (m_bucketCount - 1)) *
                                                             m_alignedBucketSize);
        }
    ...
    void Remove(const Key &key)
    {
        ...
        while (true)
        {
            Bucket *nextBucket = reinterpret_cast<Bucket *>(
                m_data + ((index + 1) & (m_bucketCount - 1)) * m_alignedBucketSize);
    ...
    Iterator Begin() { return Iterator(reinterpret_cast<Bucket *>(m_data)); }
...
private:
    bool Insert(Key &&key, Value &&value, uint8_t *data, size_t bucketCount)
    {
        ...
        while (true)
        {
            ...
            PSL++;
            index++;
            bucket = reinterpret_cast<Bucket *>(data +
                                                (index & (bucketCount - 1)) * m_alignedBucketSize);
        }
    ...
    Key *KeyCast(Bucket *bucket) const { return reinterpret_cast<Key *>(bucket->key); }

    Value *ValueCast(Bucket *bucket) const { return reinterpret_cast<Value *>(bucket->value); }
