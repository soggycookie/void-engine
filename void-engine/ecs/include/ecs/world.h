...
namespace ECS
{
    ...
    public:
        ...
        void SwapBackAndRemove(EntityRecord record);
                ...
                for(ArchetypeRecord& record : archetypeRecords)
                {
                    ...
                    if(edgeIter != m_componentIndex.end())
                    {
                        ...
                        if(!removedArchetype.componentSet.m_components.size() == 1)
                        {
                            ...
                            {
                                //reallocate
                                EnsureCapacity(movedArchetype, remainingSlot);
                            }
...
#define ECS_DEFAULT_COMPONENT_CAPACITY 4
        ...
        Archetype* CreateAndGetArchetype(const ComponentSet& components)
        {
            ...
            for(uint32_t i = 0; i < components.GetCount(); i++)
            {
                ...
                AllocateArchetypeColumn(*newArchetype, newArchetype->data.columns[i], ECS_DEFAULT_COMPONENT_CAPACITY);
            }
...
#define ECS_ADDED_GROW_RATIO 0.5f
        ...
        void AllocateArchetypeColumn(Archetype& archetype, ComponentColumn& column, uint32_t capacity)
        {
            ...
            size_t totalSize = capacity * column.typeInfo.size;
#if defined(_MSC_VER)
            data = static_cast<uint8_t*>(_aligned_malloc(totalSize, column.typeInfo.alignment));
#else
            data = static_cast<uint8_t*>(std::aligned_alloc(column.typeInfo.alignment, totalSize));
#endif       
            if(!data)
            {
            ...
            if(column.colData)
            {
                uint8_t* srcAddr = column.colData;
                uint8_t* destAddr = static_cast<uint8_t*>(data);

                if(column.typeInfo.isTriviallyCopyable)
                {
                    std::memcpy(destAddr, srcAddr, archetype.count * column.typeInfo.size);
                }
                else if(column.typeInfo.isMoveContructible)
                {
                    if(column.typeHook.move)
                    {
                        for(uint32_t element = 0; element < archetype.count; element++)
                        {
                            uint8_t* elementSrcAddr = srcAddr + element * column.typeInfo.size;
                            uint8_t* elementDestAddr = destAddr + element * column.typeInfo.size;
                            column.typeHook.move(elementDestAddr, elementSrcAddr);

                            if(!column.typeInfo.isTriviallyDestructible && column.typeHook.dtor)
                            {
                                column.typeHook.dtor(elementSrcAddr);
                            }
                        }
                    }
                    else
                ...
                {
                    if(column.typeHook.copy)
                    {
                        for(uint32_t element = 0; element < archetype.count; element++)
                        {
                            uint8_t* elementSrcAddr = srcAddr + element * column.typeInfo.size;
                            uint8_t* elementDestAddr = destAddr + element * column.typeInfo.size;
                            column.typeHook.copy(elementDestAddr, elementSrcAddr);
...
#if defined(_MSC_VER)
                _aligned_free(column.colData);
#else
                std::free(column.colData);
#endif    
            }
            else
            {
    ...
    private:
        ...
        std::unordered_map<ComponentId, EntityId> m_componentEntities;
