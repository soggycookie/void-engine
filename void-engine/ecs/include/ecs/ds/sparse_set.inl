...
{
template <typename T>
void SparseSet<T>::Init(WorldAllocator *allocator,
                        BlockAllocator *pageAllocator, uint32_t defaultDense,
                        bool reservedFreeId)
{
...
SparsePage<T> *SparseSet<T>::CreateSparsePage(uint64_t id)
{
    ...
    SparsePage<T> *newPage =
        PTR_CAST(m_sparse.GetElement(pageIndex), SparsePage<T>);
...
void SparseSet<T>::CallocPageDenseIndex(SparsePage<T> *page)
{
    ...
    if (m_allocator)
    {
        page->denseIndex = PTR_CAST(
            m_allocator->Calloc(sizeof(uint32_t) * SparsePageCount), uint32_t);
    }
    else
    {
        page->denseIndex =
            PTR_CAST(std::calloc(SparsePageCount, sizeof(uint32_t)), uint32_t);
    }
...
uint32_t SparseSet<T>::PushBack(uint64_t id, U &&element, bool newId)
{
    ...
    {
        uint64_t freeId = 0;
        uint32_t nextAliveCount = m_aliveCount + 1;
        uint32_t denseCount = m_dense.GetCount();

        if (m_dense.IsReqGrow())
        {
            void *oldDense = m_dense.GetArray();
            uint32_t oldDenseSize =
                m_dense.GetElementSize() * m_dense.GetCapacity();

            // NOTE: consider this approach.
            // Allocator null make dense allocate ineffciently by increasing
            // just 1
        ...
        /*
            Dense array can have both valid and invalid id (to reuse) depend on
           reservedFreeId field

            Invalid ids are swaped to the end of the array
        */
        ...
        if (newId)
        {
            ...
            {
                PTR_CAST(m_dense.GetFirstElement(), uint64_t)
                [nextAliveCount] = id;
                m_dense.IncreCount();
            }
        }
...
template <typename T>
bool SparseSet<T>::IsExisting(uint64_t id)
{
    ...
    uint32_t denseIndex =
        CAST(page->denseIndex[GetPageOffset(lowId)], uint32_t);

    return denseIndex != 0;
}

template <typename T>
bool SparseSet<T>::IsPageExisting(uint64_t id)
{
    uint32_t lowId = CAST(id, uint32_t);
...
// template <typename T>
// bool SparseSet<T>::IsVersionOutdated(uint64_t id)
// {
//     uint32_t lowId = CAST(id, uint32_t);
//
//     if (lowId == 0)
...
//     }
//
//     uint64_t matchedId = m_dense[page->denseIndex[GetPageOffset(lowId)]];
//
//     return matchedId != id;
// }
...
T *SparseSet<T>::GetPageData(uint64_t id)
{
    ...
    {
        return CAST_OFFSET_ELEMENT(page->data, T, sizeof(T),
                                   GetPageOffset(lowId));
    }
    else
    {
...
void SparseSet<T>::Remove(uint64_t id)
{
    ...
    if (m_aliveCount > 1)
    {
        if (m_reuseId)
        {
            if (denseIndex != m_aliveCount)
            {
                SwapDense(denseIndex, m_aliveCount, true);
            }
        }
        else
        {
...
uint32_t SparseSet<T>::GetDenseIndex(uint64_t id)
{
    ...
    return page->denseIndex[GetPageOffset(id)];
}
                             ...
                             bool swapPageDense)
{
    ...
    uint64_t destId =
        *CAST_OFFSET_MEM_ARR_ELEMENT(m_dense, destIndex, uint64_t);
...
{
    for (uint32_t i = m_aliveCount + 1; i < m_dense.GetCount(); i++)
    {
        std::cout << PTR_CAST(m_dense.GetArray(), uint64_t)[i] << ", ";
    }
    std::cout << std::endl;
...
uint64_t SparseSet<T>::GetReusedId()
{
    ...
    // + 1 is for id 0
    // id 0 does not count as alive
    if (m_dense.GetCount() <= (m_aliveCount + 1))
    {
        return 0;
    }

    uint64_t id =
        *CAST_OFFSET_ELEMENT(m_dense.GetArray(), uint64_t,
                             m_dense.GetElementSize(), m_aliveCount + 1);
...
{
    for (uint32_t i = 1; i <= m_aliveCount; i++)
    {
    ...
    {
        SparsePage<T> *page =
            CAST_OFFSET_MEM_ARR_ELEMENT(m_sparse, i, SparsePage<T>);
        ...
        if (page->denseIndex && page->data)
        {
            if (m_allocator)
            {
                m_allocator->Free(SparsePageCount * sizeof(uint32_t),
                                  page->denseIndex);
            }
            else
            {
