...
struct Query
{
    ...
    CallbackArg GetExecCallbackArg(const QueryIter &iter,
                                   QueryArchetype &matched, uint32_t sigIdx,
                                   uint32_t eIdx);

    template <typename CallbackArg>
    CallbackArg GetFilterCallbackArg(const QueryIter &iter,
                                     QueryArchetype &matched, uint32_t sigIdx,
                                     uint32_t eIdx);
...
public:
    template <typename... CallbackArgs>
    QueryCallBackBuilderBase<Derived, Handle> &Filter(bool (*)(CallbackArgs...),
                                                      void *ctx = nullptr);
...
private:
    ...
    QueryCallbackBuilder(Query *query)
        : QueryCallBackBuilderBase<QueryCallbackBuilder, QueryHandle>(query)
    {
    }
};
