...
void TypeInfoBuilder<T>::Register()
{
    ...
    // Get new id to avoid version mistmatch of meta entity
    if (world->IsEntityExist(ti.id))
    {
        ti.id = LO_ENTITY_ID(world->GetNewId());
    }

    ComponentTypeId<T>::Id(ti.id);
    world->Register(ti, id, GetComponentName<T>());
